...
def _build_full_route_nodes(
    ...
    for vid, route in plan.routes.items():
        ...
        for req in route.stops:
            ...
            # if not enough space - return to depot
            if cap_left < demand:
                if cur != plan.depot:
...
def export_plan_to_html(
    ...
    for algo_name, plan in plans_by_algo.items():
        ...
        # which vehicle handles which delivery node: node_id -> vehicle_id
        ...
        # arrival time of vehicles to their delivery nodes
        deliveries_meta = []
